...
//     // 수은주 높이 및 색상 계산 로직 (1단계 100% -> 12단계 10%)
//     const heightPercent = 100 - ((pageNum - 1) / 11 * 90);
//     const ratio = (pageNum - 1) / 11;
...
//         setTimeout(() => {
//             // 수은주 하강 애니메이션 시작
//             column.css({ 'height': heightPercent + '%', 'background-color': color });
//             bulb.css('background-color', color);
            
//             // 3. [수정] fadeOut 없이 연출 완료 후 즉시 페이지 이동
//             setTimeout(() => {
//                 const nextIdx = pageNum + 1;
//                 SetFridgeCookie('fridge_idx', pageNum);
                
//                 if (pageNum > 12) {
//                     window.location.href = 'ending.html';
//                 } else {
//                     window.location.href = `question_${pageNum}.html`;
//                 }
//                 // 여기서 .fadeOut()을 호출하지 않으므로 온도계가 화면을 가린 채 페이지가 바뀝니다.
//             }, 2000); // 수은주가 내려가는 것을 지켜보는 시간
//         }, 300);
//     });
// };
...
//     // [계산식] 1단계 완료 시점부터 12단계까지 90%의 구간을 나눔
...
//     const startPos = getPos(pageNum - 1); // 이전 단계의 끝 지점
//     const endPos = getPos(pageNum);       // 현재 단계의 목표 지점
//     const startColor = getCol(pageNum - 1);
//     const endColor = getCol(pageNum);

//     // 2. 애니메이션 전 "시작 지점"으로 즉시 이동 (트랜지션 제거)
...
//             // 4. 연출 완료 후 "다음 인덱스"로 이동
//             setTimeout(() => {
//                 const nextIdx = pageNum + 1; // 다음 페이지 번호 계산
                
//                 // [수정] 현재 번호가 아닌 다음 번호(nextIdx)를 저장하고 이동
//                 SetFridgeCookie('fridge_idx', nextIdx);
